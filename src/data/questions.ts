...
export function getQuestionsBySection(part: string, sectionType: string): Question[] {
    return questions.filter(q => q.part === part && q.section === sectionType);
}
...
export function getSectionById(sectionId: string): Section | undefined {
    return getParts().flatMap(p => p.sections).find(s => s.id === sectionId);
}
